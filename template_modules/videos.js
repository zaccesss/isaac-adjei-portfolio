...
export function videoOptimizerPlugin(userCfg = {}, logger = console) {
  ...
  const L = {
    start: (msg) =>
      typeof logger?.start === 'function' ? logger.start(msg) : logger?.log?.(msg),
    file: (msg) =>
      typeof logger?.file === 'function' ? logger.file(msg) : logger?.log?.(msg),
    done: (msg) =>
      typeof logger?.done === 'function' ? logger.done(msg) : logger?.log?.(msg),
    warn: (msg) =>
      typeof logger?.warn === 'function' ? logger.warn(msg) : console.warn(msg),
    error: (msg) =>
      typeof logger?.error === 'function' ? logger.error(msg) : console.error(msg),
  };

...
export const projects: Project[] = [
  {
    ...
    description: "Analogue two-stage amplifier with TL071 active band-pass filter and OPA551 buffer, simulated in Proteus and manufactured as a custom PCB",
    longDescription:
  ...
  },
  {
    ...
    description: "64 individually addressable WS2812B LEDs on an Arduino Uno with adaptive brightness, four animation modes and a custom wooden enclosure",
    longDescription:
  ...
  },
  {
    ...
    description: "Full-stack CV database website built from scratch with PHP 8.2, MySQL and custom CSS - no frameworks, deployed live on Aston University's server",
    longDescription:
    ...
    technologies: ["PHP 8.2", "MySQL", "CSS3", "JavaScript", "Apache", "Composer", "mPDF", "Cloudflare"],
    category: "web",
    featured: true,
    images: [
  ...
  },
  {
    ...
    description: "Terminal-style landing page for zacess.com - behaves like a real CLI session in the browser, built with Next.js 14 and TypeScript",
    longDescription:
  ...
  },
  {
    ...
    description: "Safety-critical Arduino control system for a CNC milling machine with door interlocks, emergency stop, state machine firmware and LCD feedback",
    longDescription:
  ...
  },
  {
    ...
    description: "Arduino-based multi-floor goods lift controller with request queuing, door interlocks, emergency stop, PWM motor control and LCD status display",
    longDescription:
  ...
  },
  {
    ...
    description: "Three AutoCAD projects covering 2D technical drawing, 3D solid modeling and full product design to BS 8888 standard",
    longDescription:
  ...
  },
  {
    id: "git-unlocked",
    title: "git-unlocked",
    description: "Free, MIT-licensed open source course taking anyone from absolute zero to industry-level Git proficiency across every major platform and tool",
    longDescription:
    ...
    technologies: ["Git", "GitHub", "GitLab", "Bitbucket", "Azure DevOps", "Markdown", "GitHub Actions", "Open Source"],
    category: "other",
    featured: true,
    images: [],
  ...
  },
  {
    ...
    description: "Full-stack IoT predictive maintenance platform with ESP32/Arduino/STM32 firmware, FastAPI backend, Isolation Forest anomaly detection and a Next.js live dashboard — actively being built",
    longDescription:
      "PHAEMOS (pronounced FAY-mos — coined from Ancient Greek roots for revelation and order) is a full-stack IoT predictive maintenance platform being built from hardware up. The tagline: Reveal before failure. It collects real sensor data from ESP32, Arduino Uno and STM32 hardware nodes, ingests it through a FastAPI backend, processes it with a machine learning anomaly detection model, and surfaces everything on a Next.js live dashboard with alert management and an automated maintenance ticket workflow.\n\nThe hardware layer uses DHT22 for temperature and humidity, MPU6050 for vibration and acceleration, LDR for ambient light, and DS18B20 for precise temperature. The ESP32 acts as the primary Wi-Fi gateway posting sensor readings every 5 seconds. The Arduino Uno reads sensors over serial and relays via the ESP32; the STM32 handles high-frequency vibration sampling over UART.\n\nThe backend is FastAPI + PostgreSQL 15 with Redis for caching and queuing, JWT authentication with role-based access control (Admin, Technician, Viewer), an alert rules engine, a maintenance ticket system and a full audit log. The ML pipeline uses scikit-learn Isolation Forest trained on normal telemetry to score each new reading 0–1 — scores above 0.7 are flagged as anomalies and trigger alerts automatically. The full stack runs with Docker Compose and deploys to Vercel (frontend) and Render (backend + database).\n\nThe project follows a 12-week milestone plan: Phase 1 covers firmware and the live telemetry dashboard, Phase 2 adds the alert rules engine, JWT auth, ticket system and audit logs, and Phase 3 delivers ML anomaly scoring and maintenance recommendations.",
    technologies: ["Next.js 14", "TypeScript", "FastAPI", "Python", "PostgreSQL", "Redis", "scikit-learn", "Docker", "ESP32", "Arduino", "STM32", "Tailwind CSS", "Vercel"],
    category: "web",
    featured: true,
    images: [],
    ...
    date: "2026",
    highlights: [
      "Full hardware stack: ESP32 (Wi-Fi gateway), Arduino Uno (serial relay) and STM32 (high-frequency vibration)",
      "Sensors: DHT22 (temp/humidity), MPU6050 (vibration/accel), LDR (ambient light), DS18B20 (precise temp)",
      "FastAPI + PostgreSQL backend with Redis, JWT auth, role-based access control and full audit logging",
      "Isolation Forest ML pipeline: scores every telemetry reading 0–1, auto-triggers alerts above 0.7",
      "Alert rules engine + automated maintenance ticket workflow with Admin, Technician and Viewer roles",
      "Full Docker Compose stack deployed to Vercel (frontend) and Render (backend + database)",
    ],
  },
]

...
const releases: ChangeEntry[] = [
  {
    ...
    date: "",
    added: [
      "/consumed page: 49 YouTube videos, 12 Spotify podcasts and 10 books logged for 2026; content sorted oldest to newest by real upload date and spread equally across January to May; All tab shows all content grouped by month; click-to-play video facade; music section links to the Notes page Spotify widget",
      "/now page: snapshot of what I am doing in my life at this moment, with sections for location, studying, building, reading, thinking about, outside of work and listening; inspired by nownownow.com",
      "/uses page: all the hardware, software and tools I use day to day",
      "/colophon page: how the site is built, the stack and the decisions behind it",
      "/changelog page: this page, a full history of the site from the first commit",
      "Dark/light mode crossfade animation (150ms ease, applies only during the toggle)",
      "Next and previous post navigation at the bottom of every blog post",
      "Blog reactions: thumbs up, flame, lightbulb and heart per post stored in Redis",
      "Post series grouping: series + seriesPart fields on BlogPost; SeriesBanner component on post pages; series indicator on post cards",
      "Hall of Fame reframe: personal acknowledgements (God, mum, dad) lead the page before security researchers",
      "Command menu now searches projects and includes all hidden pages in a More group",
      "Gaming PC daemon, API route and live card (NVIDIA RTX 4060 via pynvml, CPU%, game detection)",
      "Lenovo daemon, API route and live card (battery, charging state via NSSM service)",
    ],
    fixed: [
      "YouTube and Spotify embeds blocked by CSP: added www.youtube.com and open.spotify.com to frame-src in next.config.mjs",
      ...
      "Separator and icon visibility improved in both light and dark mode using foreground-based opacity",
    ],
  },
  {
    version: "v2.2.0",

...
const releases: ChangeEntry[] = [
  {
    ...
    added: [
      "A new /stats section: a public hub page linking into GitHub, coding, music, applications and gaming stats, each on its own page. GitHub, coding and gaming stats moved here from /lab, which now just has the terminal and PCB viewer. GitHub gained a year-by-year contributions bar chart alongside its usual calendar, plus a period-filterable daily activity chart (a bar per day and a running total line). Music gained real listening history - a calendar of daily play counts, a radial clock of listening by hour of day, a genre treemap, a release-decade bar chart and a word cloud of top artists, most filterable by period - alongside the existing top tracks, artists and genres. Applications gained a genuine map of where my job search has reached, one pin per city sized by count, count-only with no company, status, role or date behind it, an always-on total application count and a top-10-cities bar chart alongside the map - hover a pin for a quick number, nothing to click through to",
    ...
    fixed: [
      "The public applications map overstated its total as literal applications sent, when most tracked locations are listings I found and logged rather than places I have applied to - reworded the headline, popups and top-cities chart to say opportunities tracked instead. The map also gained the same style picker, globe/3D toggles and clickable clustering the dashboard's own map has",
      "The applications map's globe view left a small sphere floating in a lot of empty space, now fills the box properly with a real sky behind it. The site's light/dark toggle now carries the map along with it too. Pin labels and the top-cities chart showed the raw scraped location text, some of it genuinely uninformative - both now show a real city and country wherever the location has been geocoded",
      "The top-cities chart only showed some of its 10 labels, cut for space - hidden now since the colour legend beneath already names every bar. Applications tagged Remote, which has no city to plot, now show as their own honest Remote entry instead of silently disappearing",

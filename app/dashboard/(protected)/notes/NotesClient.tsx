...
export default function NotesClient({ notes: initial }: { notes: Note[] }) {
  ...
  const [draft, setDraft] = useState({ title: "", content: "", folder: "General", tags: [] as string[], color: null as string | null, locked: false, pinned: false })
    ...
    <div className="flex gap-4 h-[calc(100vh-8rem)] max-w-5xl">
      ...
      <aside className="w-56 shrink-0 flex flex-col gap-3 border-r border-border pr-4">
        ...
        <div className="flex flex-col gap-1">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide px-1">Folders</p>
          {folders.map((f) => (
            <button
              key={f}
              type="button"
              onClick={() => setActiveFolder(f)}
              className={`flex items-center gap-2 px-2 py-1.5 rounded text-sm text-left transition-colors ${activeFolder === f ? "bg-muted font-medium" : "text-muted-foreground hover:text-foreground hover:bg-muted/50"}`}
            >
              <Folder className="h-3.5 w-3.5 shrink-0" />{f}
            </button>
          ))}
        </div>

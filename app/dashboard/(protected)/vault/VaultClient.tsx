...
type VaultEntry = {
  ...
  content: string | null
  notes: string | null
  fields: Record<string, unknown>  // I reserve this for future arbitrary key-value expansion
}

// I centralise type metadata in one array so adding a new type only requires one change here
const VAULT_TYPES = [
...
// I map each type to a colour class so badges are visually distinct at a glance
...
function VaultForm({ initial, onClose }: {
  ...
  // I default type to "account" because it is the most common entry type
  ...
  return (
    ...
    <div className="flex flex-col gap-3 max-h-[70vh] overflow-y-auto pr-1">
      {/* I show all type options as pill buttons rather than a Select so switching type is one click */}
      ...
      {/* I conditionally render only the fields relevant to the selected type
          so the form never looks overwhelming with blank irrelevant fields */}
      {(type === "account") && (
        <>
...
export default function VaultClient({ entries: initial }: { entries: VaultEntry[] }) {
  ...
  const filtered = entries.filter((e) => {
    // I check type filter first because it is cheaper than the string search
  ...
  function handleDelete(id: string) {
    ...
    deleteVaultEntry(id)  // I do not wrap this in startTransition because there is no optimistic state to maintain
  }
  ...
  return (
    <div className="flex flex-col gap-5 max-w-2xl">
      ...
      {/* I use pill buttons for type filter rather than a Select because all five options fit on one row
          and switching between them is a single click rather than two */}

...
import { Globe, StickyNote, Key, CreditCard, User, ShieldCheck } from "lucide-react"
...
export default function VaultClient({ entries }: { entries: VaultEntry[] }) {
  ...
  const typeCounts = VAULT_TYPES.map((t) => ({
    ...t,
    count: entries.filter((e) => e.type === t.dbValue).length,
  }))
  const populatedTypes = typeCounts.filter((t) => t.count > 0).length

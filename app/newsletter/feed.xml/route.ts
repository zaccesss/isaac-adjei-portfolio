...
function buildXml(issues: NewsletterIssue[], baseUrl: string, includeStylesheet = true) {
  const items = issues
    .map((issue) => {
      const pubDate = new Date(issue.publishDate).toUTCString()
      return `
    <item>
      <title><![CDATA[${issue.title}]]></title>
      <link>${issue.webUrl}</link>
      <guid isPermaLink="true">${issue.webUrl}</guid>
      ${issue.subtitle ? `<description><![CDATA[${issue.subtitle}]]></description>` : ""}
...
<rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom" xmlns:dc="http://purl.org/dc/elements/1.1/">
  <channel>
    <title>Newsletter: Isaac Adjei</title>
    <link>${baseUrl}/newsletter</link>
    <description>Dispatches from Isaac Adjei: writing, building and learning.</description>
    <language>en-gb</language>
    <managingEditor>contact@isaacadjei.me (Isaac Adjei)</managingEditor>
    <atom:link href="${baseUrl}/newsletter/feed.xml" rel="self" type="application/rss+xml" />
    <image>
...
function buildHtml(issues: NewsletterIssue[], baseUrl: string, page = 1) {
  const totalPages = Math.ceil(issues.length / ISSUES_PER_PAGE)
  const currentPage = Math.max(1, Math.min(page, totalPages || 1))
  const paginated = issues.slice((currentPage - 1) * ISSUES_PER_PAGE, currentPage * ISSUES_PER_PAGE)

  const items = paginated
    .map((issue) => {
      const pubDate = new Date(issue.publishDate).toLocaleDateString("en-GB", {
        day: "numeric", month: "long", year: "numeric",
      })
      return `
      <div class="post">
        ${issue.thumbnailUrl ? `<img class="post-thumb" src="${issue.thumbnailUrl}" alt="" />` : ""}
        <div class="post-body">
          <div class="post-cat">${issue.status === "archived" ? "archived" : "newsletter"}</div>
          <div class="post-title"><a href="${issue.webUrl}" target="_blank" rel="noopener noreferrer">${issue.title}</a></div>
          ${issue.subtitle ? `<div class="post-desc">${issue.subtitle}</div>` : ""}
          <div class="post-meta">
            <span>${pubDate}</span>
            <span class="post-author">Isaac Adjei</span>
          </div>
          <div class="post-actions">
            <a href="${issue.webUrl}" class="post-action" target="_blank" rel="noopener noreferrer">&#128279; Read issue</a>
          </div>
        </div>
      </div>`
    })
  ...
  <head>
    ...
    <style>
      *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .post-thumb { width: 80px; height: 56px; object-fit: cover; border-radius: 0.375rem; flex-shrink: 0; border: 1px solid var(--card-border); }
    ...
    <div class="container">
      <div class="header">
        <div class="header-top">
          ...
          <div>
            <div class="site-name">Newsletter: Isaac Adjei</div>
            <div class="site-desc">Dispatches: writing, building and learning.</div>
          </div>
        </div>
        <div class="subscribe-box">
          <span class="rss-icon">&#9656;</span>
          <span>This is an RSS feed. Subscribe in your reader at <a href="${baseUrl}/newsletter/feed.xml">isaacadjei.me/newsletter/feed.xml</a> or browse <a href="${baseUrl}/newsletter">newsletter</a>.</span>
        </div>
        <br/>
        <a class="raw-btn" href="${baseUrl}/newsletter/feed.xml?raw">&#60;/&#62; View raw XML</a>
      </div>
      ...
      <p style="text-align:center;font-size:0.75rem;color:var(--muted);margin-top:0.5rem;">Showing ${(currentPage - 1) * ISSUES_PER_PAGE + 1}&#8211;${Math.min(currentPage * ISSUES_PER_PAGE, issues.length)} of ${issues.length} issues</p>` : ""}
      <div class="footer">
...
export async function GET(request: Request) {
  ...
  if (accept.includes("text/html")) {
    ...
    return new Response(buildHtml(issues, baseUrl, page), {
      headers: {
  ...
  return new Response(buildXml(issues, baseUrl), {
    headers: {

...
const sections: { icon: React.ComponentType<{ className?: string }>; heading: string; items: ColophonItem[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        name: "Spotify",
        detail: <>A Next.js API route fetches the currently playing track from the <A href="https://developer.spotify.com/documentation/web-api">Spotify Web API</A> on demand. The OAuth access token (which expires every hour) is refreshed server-side and cached in Redis, and the now-playing result itself is cached for a few seconds so that however many tabs are open, Spotify is polled at most once every few seconds rather than once per tab. Spotify retired its audio-features and audio-analysis APIs in November 2024, so the visualiser no longer tries to react to the sound - instead it extracts the dominant colours from the album art and renders soft drifting glow blooms behind organic equaliser bars on a device-pixel-ratio canvas, animated on delta-time and tuned separately for light and dark mode. Genre tags come from <A href="https://www.last.fm/api">Last.fm</A> (Spotify retired artist genres in 2025). The progress bar ticks every second client-side. Live status is streamed over a single SSE connection that reads every source in-process and pauses while the browser tab is hidden; its Spotify channel polls every 3 seconds while a track is playing so song changes appear in near-realtime, and backs off to 15 seconds when idle. When nothing is playing, the last played track is shown in a greyed-out state from a separate Redis key.</>,
      },
      {
        name: "PS5",

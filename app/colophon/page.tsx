...
const sections: { icon: React.ComponentType<{ className?: string }>; heading: string; items: ColophonItem[] }[] = [
  ...
  },
  {
    ...
    items: [
      {
        name: "/lab - interactive terminal, GitHub stats, live coding stats, top picks, gaming panel and PCB viewer",
        detail: "An in-browser terminal with 30+ commands spanning navigation, content, live stats and personality. Several commands animate theatrically line by line. The coding stats panel has a period selector, stat cards, daily trend line, 7x24 interactive heatmap with an hourly activity sparkline and peak coding hour below it, charts and hour-of-day bars. The top picks panel shows a ranked list of the top 20 tracks and top 15 artists from the last four weeks via the Spotify Web API, with album art, popularity bars and a switchable energy-vs-mood scatter plot: each dot is a track positioned by valence (x-axis, sadder to happier) against energy (y-axis) with hover labels for track and artist name. The gaming panel shows PS5 and Gaming PC status side by side - game cover art as a banner, online or last-seen badge and CPU plus GPU utilisation bars when the PC is active, updated every 30 seconds. The PCB viewer loads the actual 3D mesh of the audio amplifier board (real GLB model via react-three-fiber and drei) with angle presets, wireframe mode, auto-rotate and a grid; below it are drag-to-orbit copper layer renders, a front/back photo flip card of the built board, an assembled board photo and the full circuit schematic - the latter two open fullscreen in a lightbox on click.",
      },
      {

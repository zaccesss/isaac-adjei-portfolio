...
const sections: { icon: React.ComponentType<{ className?: string }>; heading: string; items: ColophonItem[] }[] = [
    ...
    items: [
      {
        ...
        detail: "An in-browser terminal with 30+ commands spanning navigation, content, live stats and personality. Several commands animate theatrically line by line. The coding stats panel has a period selector, stat cards, daily trend line, 7x24 interactive heatmap with an hourly activity sparkline and peak coding hour below it, charts and hour-of-day bars. The PCB viewer loads the actual 3D mesh of the audio amplifier board (real GLB model via react-three-fiber and drei) with angle presets, wireframe mode, auto-rotate and a grid; below it are drag-to-orbit copper layer renders, a front/back photo flip card of the built board, an assembled board photo and the full circuit schematic - the latter two open fullscreen in a lightbox on click. The gaming panel shows PS5 and Gaming PC status side by side - game cover art as a banner, online or last-seen badge and CPU plus GPU utilisation bars when the PC is active. The top picks panel has three tabs: Tracks shows an album art mosaic strip across all 20 tracks, audio stat pills (avg energy, mood, BPM, danceability) and a ranked list beside an energy-vs-mood scatter plot; Artists shows a circle photo mosaic then a ranked list with genre tags; Genres shows a horizontal bar chart of genre frequency across the top 15 artists with coloured bars and pill tags for the top five.",
      },
      {
  ...
  },
  {
    ...
    items: [
      ...
      {
        name: "Spotify",
        detail: <>A Next.js API route fetches the currently playing track from the <A href="https://developer.spotify.com/documentation/web-api">Spotify Web API</A> on demand. The OAuth access token (which expires every hour) is refreshed server-side and cached in Redis so it is never fetched on every single request. In addition to the track and artist, the API fetches audio features (energy, tempo, valence, danceability, loudness in dB) and a full audio analysis (beat timestamps with confidence scores, cached in Redis by track ID so they are only fetched once per track). The progress bar ticks every second client-side; the visualiser uses the audio analysis beat timestamps plus the interpolated playback position to pulse the bars at the exact moment each beat falls. Loudness drives the brightness of the bars, the sine wave amplitude and the album art tint behind the visualiser - louder tracks glow more vividly. Two SSE channels run in parallel: a dedicated Spotify-only channel polls every 5 seconds so song changes appear on screen within 5 seconds of switching tracks; a full status channel refreshes all devices every 60 seconds. When nothing is playing, the last played track is shown in a greyed-out state from a separate Redis key.</>,
      },
      {
        name: "PS5",

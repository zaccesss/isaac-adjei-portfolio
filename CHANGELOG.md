...
- A new public `/stats` section (issue #942's plan, PR 2 of the two-PR plan): a hub page linking into `/stats/github`, `/stats/coding`, `/stats/music`, `/stats/applications` and `/stats/gaming`. `GitHubStats`, `WakatimeStats`, `SpotifyAnalytics` and `GamingPanel` moved here from `/lab`, which now only has the terminal and `PCBViewer`. Two new public API routes (`/api/stats/music-history`, `/api/stats/applications-locations`) aggregate `listening_history` and `applications` down to safe, count-only shapes - no individual play, track, artist, company, status, role or date ever leaves either route. GitHub gained a year-by-year contributions `BarChart`, fed by exposing `github_contributions_years` (already synced, never surfaced) through `getStoredGithubContributions()` and `/api/github-stats`. Music gained a new `MusicHistory` component (`CalendarHeatmap` of daily plays, `RadialClock` of listening by hour, a genre `Treemap`), period-filterable through the shared `AnalyticsPeriodProvider`/`PeriodSelector` the rest of the dashboard already uses - `/api/stats/music-history` takes the same `period` query param as `/api/wakatime-stats`. Applications gets a new `PublicApplicationsMap` (OpenFreeMap only, deliberately not MapTiler, since a public page could see real traffic that would exhaust the private dashboard's shared MapTiler quota) - one pin per city sized by count with a hover tooltip, no click-through detail, plus an always-on total application count matching the dashboard's own "Total" convention (every application, scraped included). Wired into the footer, `/all-pages` and the command menu (`Mod+Y`, `i` itself being reserved for opening the palette)

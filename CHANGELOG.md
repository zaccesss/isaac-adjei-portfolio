...
- Every Linear issue this app opens now lands assigned to me with a category label instead of sitting unassigned in the team backlog: health incidents carry health plus a label naming the repo the failing check belongs to, application syncs carry career plus application, university deadline syncs carry university. The assignee is resolved through Linear's own viewer query against the API key rather than a hardcoded id, and labels are found or created on first use

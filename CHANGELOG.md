...
- Every public page with its own detail pages now names its section in the browser tab, the same convention TIL already used: Blog | the post title, Project | the project name, Notes | the note title, Consumed | the book, video or artist. TIL's own separator moved from a colon to a pipe to match

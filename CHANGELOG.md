...
- Fixed a real gap in the previous workflow-status SSRF fix: assigning `workflow = requested` inside each switch case still returned the tainted request parameter itself, just narrowed, so CodeQL kept flagging it. The switch now returns a fresh string literal per case instead, which actually breaks the taint chain
- Cleared 16 CodeQL code-scanning alerts (2 critical, 13 high, 1 medium) that surfaced from a fresh scan. Replaced regex-based HTML tag stripping in the contact form, the Bible verse route and the Spotify shows route with a linear-scan stripper, since a `/<[^>]*>/g` regex can run quadratically on adversarial unclosed-tag input and CodeQL flags the single-pass version as potentially leaving a reassembled tag behind; added an RFC 5321 email length cap ahead of the contact and newsletter forms' email regex, since neither route bounded email length before testing a regex with two adjacent unbounded-and-overlapping character classes. Closed two SSRF findings by construction rather than by dismissal: the workflow-status route now resolves the requested workflow name through a switch that returns fixed literals rather than indexing an allowlist object with the raw request param and the Discord bot's followup call URL-encodes both the app id and the interaction token. In the CV-generation script, fixed a script/style-tag strip regex that did not match a closing tag with a space before its `>`, reordered the HTML-entity decoder so `&amp;` unescapes last (decoding it first was turning a legitimately double-encoded `&amp;lt;` into a literal `<`) and wrapped every regex-based comment/tag strip in a loop that repeats until the string stops changing. Added an explicit `permissions: contents: write, actions: read` block to the CV-generation workflow, which previously ran with the default (broader) `GITHUB_TOKEN` scope
- Patched a fresh batch of Dependabot alerts: `brace-expansion` past its latest DoS patch (a new advisory landed mid-fix widening the vulnerable range to include the version I had just pinned, so I re-checked and bumped again rather than assuming yesterday's patch still held), `postcss` (both the direct copy and the one `next` bundles internally, which needed its own targeted override since `next` pins an exact version its own package.json does not expose to a plain override), `ip-address` (via `puppeteer-core`'s own dependency chain) and a batch of `undici` WebSocket and cache-related CVEs pulled in transitively through both `wrangler`/`miniflare` and, separately, `@ai-sdk/provider-utils` (fixed by bumping `wrangler` itself plus targeted overrides for both nested copies, since each package capped its own accepted `undici` range below the patch independently). Also caught and reverted an unintended side effect mid-fix: a first attempt that deleted `package-lock.json` for a clean re-resolve pulled `@ai-sdk/provider-utils` forward a patch version, which dragged in its own vulnerable nested `undici` copy alongside the already-safe root one - re-did the fix from the original lockfile with only the packages that actually needed to move, so nothing else drifted. A concurrent wave of Dependabot's own auto-merges then landed the exact same `@ai-sdk/provider-utils` version on `main` while I was mid-fix regardless, so I fixed that nested copy for real with its own override rather than leaving it. Rebasing onto that wave also broke ESLint outright (`brace-expansion.braceExpand is not a function`): a bare, unscoped `brace-expansion` override coexisting with several parent-scoped ones for the same package name stopped resolving correctly for ESLint's own dependency chain specifically, once `eslint-config-next`'s own version moved under it - replaced the bare catch-all with an explicit `eslint` ancestor entry instead, matching every other chain's own explicit scoping rather than relying on an implicit default
- The same batch of `undici` CVEs the root project just patched also affected the ps5-presence Worker's own separate `package-lock.json`, which Dependabot flags and tracks independently. Its `wrangler` had drifted stale relative to its own declared range and `miniflare`'s bundled `undici` copy needed the same targeted override as the root project. Confirmed the Worker's source still typechecks clean and dry-run deploys correctly against the bump
...
- Patched a brace-expansion ReDoS Dependabot flagged as three separate high-severity alerts, one per major-version lineage pulled in transitively by `glob`, `@sentry/bundler-plugin-core`, `@typescript-eslint/typescript-estree` and `@node-minify/core`. Pinned each nested copy to its patched version through targeted `overrides` rather than a broad `npm audit fix`, which would have cascaded into unrelated Sentry, Supabase and Radix bumps
- Patched 9 Next.js CVEs Dependabot flagged (SSRF in Server Actions and rewrites, cache confusion, a middleware/proxy bypass under Turbopack, unauthenticated disclosure of internal Server Function endpoints, two denial-of-service reports) by taking the patch Dependabot's own bump already applied elsewhere in the same run. Separately patched libvips CVEs in `sharp`, pulled in transitively at a vulnerable version by both `next`'s own optional image-optimisation dependency and `miniflare` (via `wrangler`, the ps5-presence Worker's deploy tooling) - both `next` and `miniflare` capped their accepted sharp range below the patched version, so a plain install would never have picked it up without a targeted override in both this repo's package.json and the Worker's own. Also found and fixed a real, pre-existing peer-dependency conflict while re-installing the Worker's dependencies clean (`wrangler` had moved on to expecting `@cloudflare/workers-types` major version 5, the Worker was still pinned to major version 4) - bumped it to match and confirmed the Worker's source still typechecks clean and dry-run deploys correctly against the new major version

### Added

- CODEOWNERS, SUPPORT.md, a CONTRIBUTING.md scoped to bug reports only and a YAML bug report issue form, matching the repo-hygiene standard used across other repositories
- A new public `/stats` section (issue #942's plan, PR 2 of the two-PR plan): a hub page linking into `/stats/github`, `/stats/coding`, `/stats/music`, `/stats/applications` and `/stats/gaming`. `GitHubStats`, `WakatimeStats`, `SpotifyAnalytics` and `GamingPanel` moved here from `/lab`, which now only has the terminal and `PCBViewer`. Two new public API routes (`/api/stats/music-history`, `/api/stats/applications-locations`) aggregate `listening_history` and `applications` down to safe, count-only shapes - no individual play, track, artist, company, status, role or date ever leaves either route. GitHub gained a year-by-year contributions `BarChart`, fed by exposing `github_contributions_years` (already synced, never surfaced) through `getStoredGithubContributions()` and `/api/github-stats`, plus a period-filterable `Composed` chart of daily activity (a bar per day and a cumulative line) behind the same shared `PeriodSelector` the rest of `/stats` uses. Music gained a new `MusicHistory` component (`CalendarHeatmap` of daily plays, `RadialClock` of listening by hour, a genre `Treemap`, a release-decade `BarChart` and a `WordCloud` of top artists), period-filterable through the shared `AnalyticsPeriodProvider`/`PeriodSelector` the rest of the dashboard already uses - `/api/stats/music-history` takes the same `period` query param as `/api/wakatime-stats`. Applications gets a new `PublicApplicationsMap` (OpenFreeMap only, deliberately not MapTiler, since a public page could see real traffic that would exhaust the private dashboard's shared MapTiler quota) - one pin per city sized by count with a hover tooltip, no click-through detail, an always-on total application count matching the dashboard's own "Total" convention (every application, scraped included) and a top-10-cities `BarChart` alongside it, from the same count-only endpoint. Wired into the footer, `/all-pages` and the command menu (`Mod+Y`, `i` itself being reserved for opening the palette)
- A dedicated Projects page (`/dashboard/projects`), tracking hardware builds, personal software and coursework projects separately from Goals and University/Study/Modules. Two new chart primitives, `Gantt` and `Burndown`, neither of which recharts or echarts has natively - `Gantt` reuses the `Waterfall` primitive's invisible-spacer-bar technique laid out horizontally, one bar per task from its own start to end date and a per-project detail page auto-builds one from that project's own tasks alongside a burndown of open tasks remaining. A new Finance page (`/dashboard/finance`) tracks savings and spending, with inline mini-analytics (running balance, spending by category, income versus expense by month, a net-change-by-month waterfall) rather than a separate analytics page, matching how Goals already works; `finance_transactions` has row level security enabled with no public policy since it holds real financial data. A new combined `/dashboard/analytics` hub pulls a headline slice from every domain, including the ones that previously only ever showed mini-analytics inline on a CRUD page (Goals, Projects, Inventory), plus a genuinely cross-domain Bubble chart of study minutes versus coding minutes per day, sized by that day's fitness minutes
- A dashboard-wide chart expansion: a 16-swatch shared colour palette (up from 9), 10 new reusable chart primitives in `components/analytics/` (Funnel, Waterfall, Bullet, Box plot/Histogram backed by `d3-array`, Gauge, Word cloud via `@visx/wordcloud`, Radar, Composed, Bubble, StackedArea) plus a sparkline slot on `StatCard`, wired into every page with matching data: Posts (Funnel from the read-depth data), Music (Word cloud of top artists), Strava (Bubble of distance vs pace, Box plot of pace by month), Weight-loss (Waterfall of weekly deltas), Medication/Goals/Streaks (Gauge), Modules (Radar of marks, Box plot of the mark distribution), Study (Pareto of time by subject), Coding (Composed chart with a running total), Diary/Faith (the existing `CalendarHeatmap`, not reused on either page before). Applications gets a real `Funnel` replacing an old hand-rolled bar chart that was faking a funnel shape, a new rejections-by-company Pareto and a genuine interactive map of application locations (MapLibre GL JS with OpenFreeMap tiles - no account, no card, real 3D building shapes - not Mapbox as first planned, which turned out to require a payment method just to create an account) backed by a new `location_geocodes` cache table fed by a background job in isaac-adjei-automations (OpenCage primary, automatic Nominatim fallback). A new study-hours-vs-module-mark Bubble scatter (`study_sessions.module_id`, a nullable FK plus a module picker in the logging form) and a full lab measurements feature (`/dashboard/analytics/lab-measurements`, a manual logging form and a real Bode plot for EECS frequency-response readings, two stacked log-frequency-axis LineCharts) round out the new data domains. A new cross-domain Time Allocation view (study/coding/Strava duration joined by day, a `StackedArea`) ships both as its own Analytics page and as a homepage summary section alongside a goals-completion Gauge
- A dashboard-wide polish pass alongside the chart work above, since this was the last dedicated dashboard session for a while: fixed `getDashboardSummary()` silently selecting a column that has never existed (`study_sessions.duration_minutes` instead of `duration_m`), which meant the homepage's Study card has always under-reported; Settings' export/import was missing 10 real tables including `location_geocodes` and `lab_measurements` from this same work; the Ops Analytics Treemap let one repo's cell swallow the whole chart under real data skew (two 15-minute-cadence jobs dominating the run-count total), now sized on a square-root scale while keeping real counts in labels and tooltips; the shared `Pagination` component's page numbers were not centred in their box, site-wide; the activity log paginated through thousands of rows for what is only ever a "what happened recently" check, now loads the latest 200 in one query with nothing pruned from the database itself; 37 pages capped their content width inconsistently (`max-w-2xl`/`max-w-3xl`), standardised all 41 dashboard pages to one shared `max-w-5xl`; the hand-built chart primitives (Funnel, Bullet, Gauge, Word cloud) had no hover detail at all unlike every recharts-native chart, now all do
- A fuller Ops analytics page at `/dashboard/ops/analytics`, linked from the main Ops page. A status-page-style uptime grid (one row per Healthchecks check, one coloured cell per day, worst-of-day rollup) alongside the existing 3 Overview charts (now shared between both pages through one extracted component rather than duplicated), a run duration trend, a per-repo success-rate breakdown, a Treemap of run volume by repo and a Sankey diagram of job outcomes by repo. `duration_s` and per-check `checked_at` timestamps were already stored in migration 048's tables and unused until now. Every section respects the same period selector the rest of the page already uses
- `/dashboard/control` and `/dashboard/uptime` merged into a new `/dashboard/ops` (both old routes now redirect there) - one page for watching and running every job across my six repos instead of two, plus a real historical Overview section neither page could show on its own: a status-breakdown pie, a daily success-rate trend and a per-job success-rate breakdown, all period-selectable (24h through all) via the same `AnalyticsPeriodProvider`/`PeriodSelector` every other analytics page uses. This needed real history, not just GitHub's live last-~20-runs-per-job and Healthchecks' current-status-only reads, so two new tables (`control_job_runs`, `control_check_snapshots`, migration 048) are populated by a new `control-status-sync` job in isaac-adjei-automations, dispatched by cron-ops every 15 minutes (Vercel's own cron is capped at once/day per job on this plan, far too coarse for jobs that themselves run every 15-30 minutes). A `GET /api/dashboard/control-jobs` route lets that sync job read the same `CONTROL_JOBS` list this dashboard already uses as its single source of truth, so a job added here needs no matching change in the automations repo. Added a shared `StatusLegend` explaining every dot colour used across the page (including the blue "running now" dot, which stays distinct from amber's "late" on purpose - they're different states), widened the Jobs table and dropped its truncated descriptions since the previous single-line clamp was cutting several off mid-sentence and gave the Jobs table its own section heading, which it never had. Every App action (both digests, all three syncs) now shows a coloured status dot matching that same legend instead of the syncs showing nothing and the digests showing lucide icons
- Fixed a real bug this uncovered: `discord-digest` and `weekly-digest`'s actual scheduled cron routes never wrote their send result to the config table, only the manual "Send test"/"Send now" buttons did - so the Ops page showed "Not sent yet" even on digests that had genuinely been sending correctly every day for over a week. Both cron routes now persist the same record the manual triggers already did. The three sync actions (Linear applications, Linear deadlines, Strava) never had this persistence built at all, so I added it - each now writes its own `last_*_sync` config key, read back through a new `GET /api/dashboard/ops-status` route (renamed from `digest-status`, which no longer described what it covers)
- Five new music charts on the dashboard's music analytics page: a genre Treemap, a radial 24-hour listening clock replacing the old linear bar chart and three Sankey diagrams tracing artist through genre to release era, time of day through genre and platform through artist. The time-of-day-to-genre flow needed a real extension to the music-history route, joining each play's London hour against its artist's most-played genre server-side rather than client-side. Both `Treemap` and `Sankey` join the shared `components/analytics/charts.tsx` set, matching the existing Recharts theming and colour palette. A Less-to-More legend was also added to the shared `CalendarHeatmap`/`GridHeatmap` components, which previously had no colour-meaning legend anywhere across the GitHub, WakaTime, Strava and blog read-time heatmaps that use them
- A circuit-board-styled isometric alternate view of the GitHub contributions calendar on /lab, sitting below the existing flat calendar: one extruded block per day, height and colour driven by the same relative-activity bucket the flat calendar uses, with the busiest days lit like an LED on a real board. The board itself carries two fixed PCB colour schemes, a dark board for dark mode and a cream FR4-style board for light mode, rather than reading the site's own design tokens, since real circuit-board colours do not map onto the site's blue-toned CSS variables. It opens on a fixed isometric angle but now supports drag to rotate, scroll to zoom, a flat top-down preset for a bird's-eye read and a reset button, matching the drag-and-zoom convention `components/lab/PCBViewer.tsx` already set for the only other three.js view on the site. A Less-to-More legend underneath reads from whichever palette is active
- A shared ECharts-based `CalendarHeatmap`/`GridHeatmap` pair in components/analytics, replacing five separate hand-rolled div-grid heatmaps that had each grown their own intensity-class lookup and manual hover/tooltip state: Wakatime's hour×day matrix (previously built twice, once on the public /lab page and once in the dashboard), Wakatime's daily-activity calendar, Strava's 52-week training calendar and the blog's read-time weekday×hour matrix. A `useEChartsColours` hook resolves the live computed value of each design token into a real color for ECharts' canvas renderer, since `hsl(var(--x))` only works in DOM/SVG styles, not inside a canvas draw call
- GitHub contribution history now has its own table instead of being fetched live from GitHub's GraphQL API on every cache miss: a new daily cron job backfills every year once, then keeps the current year topped up and a full year's worth of daily counts sits ready to read. Both the public /lab GitHub panel and the dashboard's coding page now read from this stored history and render it through the same shared `CalendarHeatmap` component the rest of the heatmap consolidation already uses, replacing GitHub's own rolling 52-week grid with a genuine year and dropping their last hand-rolled div-grid calendars. The public stats route also swaps its GraphQL follower-count lookup for a lighter plain REST call, since GraphQL was only ever needed for the contribution history it no longer fetches live
- Two experience entries (PAL Leader at Aston, Treasurer of the Computing & Electronics Society) written now for their September start but held behind a `visibleFrom` date, on the site timeline and in every CV and role variant alike. A new weekly cron-ops job regenerates the CVs as a safety net, since nothing else would trigger a rebuild once the date passes with no other content change
- A share button on every notes page and every consumed item, matching what blog posts, TIL entries and projects already had
...
- Six new entries on /consumed/resources: Tech Interview Handbook, GIPHY Developers, Sprite Sheet Generator, NotebookLM, MIT's The Missing Semester and Codédex
- Notes moved off three hand-rolled static pages onto a real /notes/[slug] route backed by data/notes, one file per entry like blog and TIL already work. The World Cup 2026 predictor note is rewritten: the tournament happened without it (Spain won), so it now covers what actually happened and the plan to generalise the same model architecture into an open-ended multi-sport platform - football, basketball, tennis, cricket, motorsport, rugby, athletics and volleyball to start, with more sports added over time rather than a fixed list. The old URL still works and redirects to the new one
- /respub: each publication now has its own detail page with the full abstract, keywords and a proper citation section (an APA-style reference plus a copyable BibTeX block), leaving the listing page as a lighter teaser with a link into the detail page
- EWskills joined the Competitive section on /links, alongside LeetCode, Codeforces and AtCoder
- Prev/next navigation between items on every /consumed detail page and every music artist page, in the same newest-first order the listing pages use
- Book and podcast cards on /consumed, matching the shared card component every other category already had - closes a real gap where a book or podcast in the "All" tab had no working link to its own detail page
- A persistent category tabs bar across the whole /consumed section: it used to only exist on the hub and disappeared entirely once you switched to a category subpage, now every subpage shows the same bar so switching category never loses it
- Year and Month filters for /blog, /til, /projects and /newsletter, built alongside a shared `monthsFromDates` helper - kept behind a feature flag in lib/feature-flags.ts until each page has enough content across enough years or months for the filter to be worth showing
- A Top 10 cities chart on the dashboard's own Applications analytics page, matching the one already on the public `/stats/applications` page, next to the existing coarser London/Birmingham/Manchester/Remote/Other breakdown

### Fixed

- The new public `/stats/applications` page overstated its total as literal applications sent - checked live against the real database, 13,785 of 13,800 rows are scraped job listings I discovered but never applied to. Reworded the headline, the map's hover popups and the top-cities chart's bar labels to "opportunities tracked" instead. Also brought the public map up to parity with the dashboard's own: the same Bright/Streets/Light/Dark/Satellite style picker plus globe and 3D tilt toggles, plus a clustered/unclustered view toggle where clicking a cluster now zooms in like the dashboard's does, grouping nearby cities into a single combined circle at low zoom rather than the individual-application clustering the dashboard uses (this endpoint only ever returns one row per city). The shared style set including the hand-built satellite-plus-labels hybrid now lives in one place (`lib/map-styles.ts`) instead of being duplicated across both maps
- Both application maps' globe view left a small sphere floating in a lot of plain white space rather than filling the box. Neither globe nor a pitched 3D view had any sky configured either, so the void beyond the sphere or above the horizon showed the page's own background colour instead of a real backdrop - globe mode now eases the zoom in on toggle, plus both views get a real sky via MapLibre's `setSky()`, re-applied after every style reload. The site's own light/dark toggle also now carries the map along with it whenever it is sitting on the Light/Dark style pair, rather than leaving a manually-picked style stuck once clicked. Pin labels and chart categories showed the raw scraped location string verbatim, which checked against the real database includes genuinely uninformative values (a job board's own "N Locations" placeholder, semiconductor fab codes) and at least one confirmed wrong existing geocode ("Munich (DEU)" resolved to Spain) - `geocode-locations.mjs` (isaac-adjei-automations) now extracts a real city and country code from the same geocode response already used for lat/lng. Both applications maps show that instead, falling back to the raw string only where geocoding has not run yet
- The Top 10 cities chart only rendered some of its 10 category labels, silently dropping whichever ones did not fit rather than rotating or shrinking them - hidden entirely now, since the chart's own colour-coded legend beneath it already names every bar. Also checked the real database behind the geocoding fix above and found something the fix alone would have quietly hidden: nearly a thousand applications tagged "Remote" or a close variant, which genuinely has no city to plot, were about to just vanish from the chart entirely rather than misleadingly attribute to a real place. Both the public and dashboard Top 10 cities charts now show "Remote" as its own honest, non-geocoded bucket alongside the real cities - the map itself still cannot plot it, since it has no coordinate at all
- Fixed a batch of real issues I found testing the Projects/Finance/All Analytics PR live in production right after it deployed: scraped rows were counting as real applications in every time-series count I had (filtered `status <> "scraped"` out of all of them), the Time Allocation colour palette was not colourblind-safe (replaced with the Okabe-Ito colourblind-safe palette), Finance was not PIN-gated like Vault/Diary/Notes already are (added the same gate) and the applications map's tile styles completely ignored my site's light/dark theme (added a dark style and default to it in dark mode). Gave All Analytics a real period selector (it shipped with a hardcoded 30 days), covered every domain that was missing from it (Study, Faith, Streaks, Habits, Weight-loss, Vault, Wishlist) and added inline mini-analytics to Wishlist, Reminders and Files, none of which had any before. Overhauled the applications map: hover tooltips, a real satellite tile option (Esri's free World Imagery service), an OpenFreeMap "bright" style for clearer country/place labelling, 2D/3D and globe toggles, popup links straight to the real job listing URL and grid-based marker clustering so the DOM marker count stays low as my applications list grows
- `generate-cvs.yml`'s weekly safety-net regeneration has failed on every single run since at least May, never once landing its changes: it committed locally then tried a direct `git push` to `main`, which the branch's own ruleset (a PR required for every change, no bypass actors) has always rejected outright regardless of the workflow's own `contents: write` permission. Brought it in line with `cv-pdf.yml`'s already-working sibling pattern - opens a branch, PRs it and auto-merges with `GH_PAT`. The default `GITHUB_TOKEN` cannot push to `main` either and its own PRs never trigger the required "Lint and Build" check needed for auto-merge to actually fire
- Added this repo's own 6 Healthchecks slugs (`vault-expiry`, `github-contributions-daily-sync`, `discord-digest`, `trash-cleanup`, `strava-sync`, `weekly-digest`) to the Linear incident label map by hand, since they come from this repo's own Vercel Cron API routes rather than a dispatchable GitHub Actions workflow and could never come from `CONTROL_JOBS`. Caught a real slug collision while doing it: this route's own daily `github-contributions-sync` ping shared its exact slug with isaac-adjei-automations' unrelated every-3-hours top-up job of the same name, which would have made their Linear incidents indistinguishable since Healthchecks checks are named globally, not scoped per repo - renamed this one to `github-contributions-daily-sync`
- The three App action sync buttons' last-synced writes used a fire-and-forget `void supabase...upsert(...)` call rather than an awaited one, so the write could get cut off once the response returned on a serverless runtime before it ever reached the database - confirmed live, the sync's own success message showed a real result while the status line still read "not synced yet". All three (Linear applications, Linear deadlines, Strava) now await the write
- Added support for a third Healthchecks project, `portfolio` (the site's own Vercel-cron pings via `HEALTHCHECK_PING_KEY`, previously invisible to this page entirely), alongside the existing `automations` and `fleet` ones. Gated on a new `HEALTHCHECKS_PORTFOLIO_API_KEY`, same read-only pattern as the other two
- Two dispatchable jobs were missing from `CONTROL_JOBS` entirely: `github-contributions-sync.yml` was added to isaac-adjei-automations weeks ago but never registered here, so it never showed on the Ops page's job table and its Healthchecks incidents carried no repo/job label, only the generic `health` one. Added it and registered the new `control-status-sync.yml` the same way so it does not repeat the same gap on day one. Also added a `repo-ops-digests` entry to the Linear incident label map directly, since it is repo-ops' own internal Worker cron rather than a GitHub Actions workflow and can never come from `CONTROL_JOBS`
- `medication-reminders`' `CONTROL_JOBS` entry still pointed at its old, now-deleted Healthchecks check (merged into `reminders`' check to free a slot for `control-status-sync`, see isaac-adjei-automations) - the Ops page's health pill for that job would have shown nothing. Repointed it at the shared `reminders` slug
- My in-dashboard AI assistant and digest summaries were carrying several stale or dead model references. GitHub Models (a free provider I used for a couple of fallback slots) was fully retired on 2026-07-30, so I removed it outright rather than leave dead entries in the picker. Gemini's default model moved from `gemini-2.5-flash` to the newer `gemini-3.6-flash`, GLM moved from 4.6 to my provider's current 5.2, my own Claude picks were still pinned to old 4.x-era ids despite Fable already being on the current naming and every one of my four OpenRouter free-tier picks (Qwen3, Llama 3.3, Gemma 4) had been pulled from OpenRouter's free tier entirely - confirmed live against the real catalog before replacing them with what is actually free there now, plus a genuinely new free Groq preview model I hadn't picked up yet. Verified every id I changed directly against a live source (the provider's own docs or API) rather than trusting a single secondhand summary, after one first pass turned up conflicting claims
- My GitHub, WakaTime and Strava calendar heatmaps (shared `CalendarHeatmap`) had quietly lost the horizontal scroll I used to have on mobile: the chart rendered at a fixed 100% width, so a full year of 12px cells squashed down to fit my phone screen instead of staying at real size and scrolling, the way GitHub's own contribution graph behaves. The chart now reports its true content width (weeks × cell size) inside a horizontally scrollable wrapper and opens already scrolled to today rather than to the oldest day in range, matching GitHub's own default
- I finished a period-selector-consistency audit I'd parked from an earlier session, going back through every dashboard page against the shared period selector and turning up several real gaps. Study, Habits, Streaks and my Coding page's own chart-window logic silently gave "All time" the identical trailing window as "1y" (usually 365 days), so switching between them showed the same chart once I had more than a year of history; each now spans back to the earliest date I actually have instead. My music page's Spotify-sourced sections (top tracks, artists, genres) can only ever reflect 3 real windows since Spotify's own API has no finer granularity, so 24h/7d/30d looked identical to each other and so did 1y/all; the section captions now say so honestly instead of implying they follow my exact selected range. My Coding page's two calendar heatmaps always render a full year regardless of the period selector sitting right above them, with nothing saying so, unlike my Strava heatmap which already discloses its own fixed window in its caption; both now carry the same kind of caption. I added a `StatCard` `scope` prop that shows a small "current"/"all-time" badge on any stat that does not follow the page's period selector, a real visual distinguisher rather than relying on wording alone, applied consistently across every such card on Habits, Library, Body Metrics, Weight-loss, Medication-reminder, Open-source, Faith, Streaks and Study
- Music genre data was unreliable: Last.fm's community tags mix real genres with bare nationality tags and my own most-played artist resolved to "french" as its top tag rather than anything genre-related. Neither MusicBrainz nor Discogs plain-text artist search could reliably fix this either, both surfacing unrelated musicians for a common name in testing. Genres now come from a union of two sources: Last.fm's tags as before, plus MusicBrainz's own curated genre taxonomy, cross-referenced not by name but by the artist's exact Spotify artist id through MusicBrainz's indexed relationship to that Spotify profile URL, which resolves to the correct artist every time. Neither source overrides the other, since an artist can carry a real, high-confidence genre only one of the two knows about; a shared junk-tag filter (extended to catch hyphen and underscore variants, not just exact matches) and the existing genre-key normaliser apply identically to both before they merge, so spelling and casing variants of the same genre collapse into one entry rather than duplicating
- The GitHub contributions calendar (public /lab and the dashboard's coding page alike) had started rendering the current calendar year instead of a genuine trailing 365 days. GitHub's `contributionCalendar` returns one entry per day across the whole requested range, including days later in the year that have not happened yet and the sync job was storing those future zero-count rows along with the real ones. The calendar's own "latest date in the data" anchor then picked up a fake date months in the future, shifting the whole displayed window. The sync no longer stores days beyond today and the read query carries the same guard defensively, so a past sync's leftover future rows can never surface again either
- The WakaTime daily coding-activity heatmap at the top of the dashboard's coding page had no title at all, unlike every other chart on the page (including the GitHub contributions heatmap right below it). Also added a small "isometric view" label above the isometric 3D calendar on the public /lab page, since it previously shared its caption with the flat calendar above it with nothing distinguishing the two
- Found two real bugs in the shared `useEChartsColours()` hook while building the isometric calendar view above. It wrapped every resolved CSS custom property in `hsl(...)` using the space-separated syntax the design tokens are actually written in, which ECharts' own colour parser cannot read, silently falling back to solid black for the zero-activity and busiest-day colours on every calendar and grid heatmap on the site, dashboard and public alike, since Phase 0.1 shipped - it now emits the older comma-separated form the parser expects. Separately, switching the site's light/dark toggle left every chart's colours one theme behind: the hook read `getComputedStyle` in the same tick the theme class flipped, before the browser had recalculated styles for the new class, so a toggle kept applying the previous theme's colours until a full reload. The read is now deferred to the next animation frame, after the class change has actually painted
- Linear issues this app opens were missing their category and repo/job labels far more often than expected and 32 older issues across both teams (Careers and Isaac Adjei) had never been assigned to me at all. Two separate root causes: `labelsForCheckName` compared Healthchecks' real check names (spaced, "Vault expiry check") against `hcSlug` values that are dash-case ("vault-expiry-check"), so any multi-word check name never matched and permanently lost its repo/job label - only single-word or already-hyphenated names happened to work. Two checks ("Routine checklist", "cron-ops scheduler") also turned out to be configured in Healthchecks under a name that matches neither their hcSlug nor their label field, so I added explicit aliases for those two. The rest of the gap was simply issues created before the auto-assign-and-label feature existed at all. Backfilled every affected issue (labels and assignee) across both teams directly and consolidated the three separately-written GraphQL callers this integration had grown (`lib/linear-common.ts`, `lib/incident.ts`, `lib/linear-sync.ts`, each with its own no-retry fetch-and-swallow pattern) onto one shared, retried caller, so a burst of near-simultaneous incidents tripping Linear's own rate limit no longer permanently drops a label the way it used to
- Two more heatmap bugs surfaced right after the previous fix. Every grid (hour-by-day and calendar alike) still rendered as basically one flat colour: I had a linear scale running from 0 to that grid's single busiest cell, which compresses every other real value into the bottom sliver of the range, so a genuinely active cell reads almost identically to an empty one unless it happens to be the all-time peak. Colouring now buckets each cell into one of 5 levels relative to its own max (the same percentage thresholds the old hand-rolled heatmaps used) instead of interpolating linearly, so a cell's colour reflects how active it was relative to the rest of the grid, not how far from the single outlier it sits. Separately, `GridHeatmap`'s day-of-week axis showed Sunday at the top and Monday at the bottom - a category y-axis in ECharts renders its first entry at the bottom, the opposite of a category x-axis, so a Monday-first array put the week in reading order upside down
- /dashboard/applications and its analytics page loaded slowly as the tracked table grew past ~9,000 rows (7,557 of them scraped job listings). Both pages paged past PostgREST's 1000-row cap by awaiting one 1000-row batch at a time in a loop - a growing table meant a growing number of sequential round trips on every single page load. Both now fetch a row count first, then every batch in one parallel burst, closing the same gap in the blog analytics events query and the music history route while I was at it, since both page the same way. Also added the useMemo the applications table's search/filter/grouping pipeline never had, so typing in the search box no longer re-runs dedupe, filtering and category grouping over the full dataset on every keystroke
- The new shared heatmaps had two live bugs right after shipping. Some cells rendered as a solid, uncoloured block instead of a real colour - a NaN or null slipping through an upstream sum (Wakatime's hour-by-day matrix summed `row.hours[h]` without a null guard) reaches ECharts as an unmappable value, so both CalendarHeatmap and GridHeatmap now coerce every value to a real number before it reaches the chart. Separately, the "zero activity" cell colour (`muted`) sat only 5 percentage points of lightness from the surrounding card in dark mode, reading as a near-solid black grid; it now uses `border` instead, which keeps a consistent, visible gap from the card in both themes. Strava's calendar also lost its orange identity in the migration - it now carries its own orange-tinted scale again instead of the shared blue default
- The lab's genre chart could show a Last.fm tag that was not a genre at all, like an *arr media-manager batch label someone had tagged onto an artist by mistake, when it was the only tag Last.fm had for that artist. Real genre tags are always space or hyphen separated and never snake_case, so any tag containing an underscore is now dropped alongside the existing exact-match junk list
- The homepage's Today I Learned preview showed whichever 3 published entries happened to sit first in the data file rather than the 3 most recent. It never sorted by date before slicing, so two old entries could sit ahead of a genuinely new one; it now sorts newest first before taking 3
- /tags was missing notes and two whole consumed categories (podcasts have no taggable field yet, resources were simply never wired in) despite /search already covering them; resources now join tags the same way book genres already do. /sitemap.xml had the same gap plus a few of its own: three notes were hardcoded by URL instead of generated from data (a fourth note would have silently missed the sitemap), publications had no individual entries at all and the tag-slug list it builds separately from the /tags pages had the same missing categories. Every content type with its own detail page now generates its sitemap entries and tag entries directly from its data file, so a new post, note, publication or consumed item just works without a second place to remember to update
- /all-pages: the Main section had drifted out of sync with the header (TIL was missing entirely, Projects and Experience were swapped); it now matches the header exactly and the old flat 17-item "More" list is split into Explore, Find things, About the site, Policies and community and Status
- Moved the site guard back to the edge runtime. During the Next 16 upgrade the maintenance-and-auth guard was renamed from middleware.ts to proxy.ts, but proxy is locked to the Node.js runtime, so a check that runs on nearly every request across the whole public site quietly moved from a near-free edge isolate to a billed Node.js function on every page view. It had become the bulk of a month's Vercel Fluid Active CPU (67 percent of the total, a 95/5 Node-versus-edge split). Renaming the file back to middleware.ts restores the edge runtime with the logic, the matcher and the exported behaviour all unchanged
...
- Swept every blog post, TIL entry and consumed-page link for rot. A dozen references had gone dead since they were written (renamed docs, retired courses, pages that moved domains, one wrong DOI, one placeholder GitHub Pages link that was never actually live) and now point at the correct current source, an equivalent replacement or, where nothing suitable exists, plain text with no dead link. Today's real-time-data post had the wrong YouTube video id and a 404'd AI SDK docs link, both fixed. The Sky Celebration Day post and TIL are reworded to reflect that a red weather warning moved the day to fully virtual on the day itself and the post is live again
- /consumed items carried a hardcoded 2026 everywhere a year was shown instead of a real per-item year field, so the whole section would have quietly read wrong from January. Every entry now carries its actual year, the hub's "All" tab sorts by real year and month instead of always starting from January regardless of year and every listing and detail page shows the year it actually happened
- Tags wiring across /consumed had several real gaps: video, article and other-link tags plus the resource category chip were plain text instead of links into /tags, book and resource detail pages had the same problem and music artists had no taggable field wired into /tags or /sitemap.xml at all despite having a genre. All of these now link through properly
- Dropped the "Ongoing" badge from Phaemos and avr-zac on /projects now that their date range alone carries that meaning
- Trimmed the newsletter page's "explore the rest of the site" block at the bottom, which had grown to nine links, all already reachable from the header or footer

### Changed

- The About page's Societies & Memberships section now supports more than one role per society over time, each with its own period and an optional `visibleFrom` gate, the same pattern `data/experience.ts` already uses. ESOC now lists both my existing Member & Student Representative role and my incoming Treasurer role (gated to 2026-09-01) and every period gained its start month rather than just a year. I also added Aston Computer Science Society (ACSS) and Aston Gaming Society (GS) to the list, rearranging all six into IET, ESOC, ACSS, Gaming Society, ACS, then AGS. The page also needed the same hourly `revalidate` the Experience page already has, since without it a `visibleFrom`-gated role would stay hidden past its date until the next deploy
...
- Blog posts and project pages now name their section in the browser tab too, matching how TIL already did it: Blog | the post title, Project | the project name, TIL | the entry title (TIL's own separator moved from a colon to match). Notes and Consumed detail pages picked up the same convention in this update: Notes | the note title, Consumed | the book, video or artist
- Logged the actual GitHub API error when the GraphQL call behind followers and the contribution graph fails, instead of silently defaulting to 0 and an empty graph with nothing in the logs to say why
...
- The dashboard's per-item pages (a note's folder, an inventory item, a vault type, a wishlist or goals category, a health section, a module year) now carry their own title in the browser tab too, instead of every item within a section sharing one generic tab name
- The footer's secondary links reordered to Now, Lab, Notes, Consumed, Research, Tags, Search
- /security-policy and /code-of-conduct now carry a "Last updated" date right under the title, same as /privacy and /notes already did
- The Spotify now-playing widget feels more current: the shared edge cache dropped from 20s to 10s and every poller (the /now card, the homepage live-status cards, the /consumed/music widget) now polls at 8s instead of 15-30s, still sitting below the cache TTL so the extra polls stay free cache hits rather than real Spotify/Vercel calls. This is a deliberate experiment given how much of tonight was spent fixing a real Fluid CPU incident: I'm watching Vercel's Active CPU usage over the next few days and will revert the TTL to 20s if it climbs (see issue #810)
